...
When a consuming choice is exercised, the archival happens automatically as part of the exercise. The Daml authorization model says that the consequences of an exercise (including the archival and any creates in the choice body) are authorized by the **actors** (controllers) of that exercise **plus the signatories** of the contract being exercised. This is why a choice body can create new contracts or archive the current one without needing additional `submit` calls.
...
This connects directly to Canton's [privacy model](/docs/canton-network/concepts). At the network level, participant nodes only receive the parts of a transaction that involve their parties. Daml's signatory and observer declarations determine which parties are involved.
...
<Cards>
  <Card title="Multi-party workflows" href="/docs/daml/multi-party-workflows" />
</Cards>
