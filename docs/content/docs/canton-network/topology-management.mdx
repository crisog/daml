...
### Jointly owned namespaces

So far, every namespace has a single root key controlled by one organization. But what about the Global Synchronizer itself? No single Super Validator should control its namespace. Canton supports **decentralized namespaces**, where multiple organizations jointly own a namespace and must reach a quorum to authorize changes. This is covered in detail on the [Decentralized namespaces](/docs/canton-network/decentralized-namespaces) page.
...
| **DecentralizedNamespaceDefinition** | Establishes a namespace jointly owned by multiple organizations with a quorum threshold. | Three Super Validators create a shared namespace for the Global Synchronizer. |
...
- **Decentralized namespaces** let multiple organizations share control of a namespace through quorum-based authorization.
