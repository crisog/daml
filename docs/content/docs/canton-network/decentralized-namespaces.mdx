...
The [topology management](/docs/canton-network/topology-management) page showed how every namespace has a root key controlled by a single organization. Acme Bank owns `acme_ns`, Keystone Brokers owns `keystone_ns`, and each can manage its namespace independently.
...
The namespace itself is computed deterministically from the **initial owners**: Canton takes the fingerprints of all owner namespaces, sorts them, and hashes the result. This means the same initial owner set always produces the same namespace, and no single owner's key is the "root."
...
Once the namespace exists, topology transactions governed by it (adding a sequencer, registering a mediator, changing parameters) require at least the **threshold** number of owner signatures. Some mapping types can still require additional co-signers based on their own authorization rules. This is where Canton's [proposal workflow](/docs/canton-network/topology-management#proposals-and-multi-signature-authorization) comes in: one owner submits a proposal, others co-sign, and once the required signatures are present, the change takes effect.
