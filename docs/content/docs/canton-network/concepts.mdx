...
A **party** is a real-world actor: a person, a company, an institution. Parties do not interact with the network directly. Instead, each party is hosted on a **participant node** operated by an organization. The participant acts on the party's behalf: it stores the party's contracts, submits transactions for it, and confirms transactions that involve it.

Think of the relationship like email: you (the party) do not talk to the internet directly. Your email provider (the participant) sends and receives messages on your behalf. You trust your provider to handle your data correctly.

One participant can host many parties. A large bank, for example, might host thousands of customer parties on a single participant node. And one party can be hosted on multiple participants (called **multi-hosting**), which gives the party redundancy or lets it interact through different organizations' infrastructure.
...
flowchart TB
    ...
    p1 <--> sync["Synchronizer"]
    p2 <--> sync
```

Alice is hosted on both participant nodes. If Acme Bank's infrastructure goes down, Alice can still transact through Keystone Brokers. Bob and Charlie are each hosted on a single participant.
