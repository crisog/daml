...
No participant ever receives data it should not see. There is no central store where all views exist in the clear, and no configuration to get wrong. The encryption boundaries are determined by the Daml contract's stakeholder rules.
...
Now that you understand how privacy works at the transaction level, the next page explains the protocol that coordinates all of this: how messages flow between participants and synchronizers to achieve consensus.
