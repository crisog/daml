...
Canton transactions involve multiple independent organizations, each running their own node. This page explains the protocol that coordinates them: how a transaction request becomes a committed (or rejected) result.
...
Now that you understand how the protocol works, the next page takes a closer look at the synchronizer components (sequencers and mediators) and what they are responsible for.
