...
flowchart TB
    ...
    subgraph p3 ["Participant Node — Bank"]
        BK["Bank's contracts"]
    end
...
In most blockchain systems, every node sees every transaction. Canton takes the opposite approach: a participant only sees the parts of a transaction that involve its own parties. If Alice and Bob trade, the Bank only learns about the parts that affect its contracts.
...
Now that you know the overall shape, the next page introduces the core concepts behind Canton: how parties, contracts, and transactions work together.
